...
// Url de la requête.
const url =
	"https://raw.githubusercontent.com/Romain-Muller/Le-pendu/main/liste_francais.txt";

// Mécanique du jeu du pendu.
async function genererMot() {
	//Récupérer les mots provenant d'une liste au format .txt hébergée sut github et en piocher un haut hasard.
...
// //Générer un nombre aléatoire tant que ce dernier nous ressort un mot beugué de la liste ou un mot avec espace ou avec un tiret.
function motAuHasard() {
	do {
		nombreAleatoire = Math.floor(Math.random() * listeDeMots.length);
	} while (
		listeDeMots[nombreAleatoire].match("�") ||
		listeDeMots[nombreAleatoire].match(" ") ||
		listeDeMots[nombreAleatoire].match("-")
	);

	motDecoupe = listeDeMots[nombreAleatoire].split("");
}
	...
	touchesVirtuelles.forEach((touche) => {
		touche.addEventListener("click", (e) => {
			...
			informations.textContent =
				"Salameche choisit la lettre " + toucheContent + " !";
			for (let i = 0; i < motDecoupe.length; i++) {
				if (lettresJustes[i].textContent == toucheContent) {
					juste(i);
					touche.style.color = "green";
					document.querySelectorAll(".vieMewtwo .vie")[
						hpMewtwo
					].className = "slideVieLeft";
					victoire();
				} else {
...
function verifierFrappe() {
	document.addEventListener("keydown", (event) => {
		...
		for (let k = 0; k < touchesVirtuelles.length; k++) {
			if (
				event.key == touchesVirtuelles[k].textContent &&
				touchesVirtuelles[k].style.color != "green"
			) {
				vie--;
				verificationVie();
...
function verificationVie() {
	switch (vie) {
		...
		case 4:
			...
			setTimeout(() => {
				informations.textContent =
					"Attention Salameche n'a plus que " + vie + " pv !";
			}, 850);
			break;
		case 3:
...
function initialiserVieMewtwo() {
	...
	vieMewtwo.style.gridTemplateColumns =
		"repeat(" + motDecoupe.length + ", 1fr)";
